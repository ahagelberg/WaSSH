...
- [x] Provider loading `OptionsDialog.tsx:196-203` vs `AiAgentProviderDialog.tsx:80-107`; `checkSelected` ≈ `refreshSelected`. Resolved: the provider editor moved into the plugin as `ProviderSettings.tsx` (§5); `OptionsDialog` no longer loads or knows about providers. The `checkSelected`/`refreshSelected` near-duplication remains inside the plugin (not core) — left as-is.
...
- [x] Break core→ai-agent coupling. Fixed with two generic capabilities: (1) `ctx.listPlugins()` on `PluginMainContext` + the optional `PluginMainModule.getSettingsSchema(ctx)` hook (with `ctx.listOtherPlugins()`), so a plugin can build schema fields from other plugins' contributions without importing them; (2) a real `settingsPresentation: 'view'` render path — `PluginRendererRegistration.settingsView` + `getPluginSettingsView()`, rendered by `OptionsDialog` as a **Configure…** button that opens the plugin's editor in a modal. Moved `AiAgentProviderDialog.tsx` → `plugins/builtins/ai-agent/ProviderSettings.tsx` (imports only `@plugin-api/*` + local files); ai-agent's manifest now sets `settingsPresentation: 'view'` and drops the `configureProviders` action field; its main module implements `getSettingsSchema` to append one permission group per other plugin's declared API. Deleted from `OptionsDialog.tsx`: all ai-agent imports, `populateAiAgentSchema`, the `ai-agent` branches, `aiAgentProviders`/`loadAiAgentProviders`/`providerDialogOpen`, and the core-owned dialog. `main/plugins/builtinRegistry.ts` no longer aggregates `OTHER_MANIFESTS` or imports `buildExternalApiPermissionField`. Moved `.ai-agent-provider-dialog-*` CSS from the core stylesheet into the plugin's. (verified: zero `plugins/builtins` imports under `src/renderer/src/{App.tsx,components/**,plugins/registry.ts}`; `src/main/**` imports plugins only in the composition root, uniformly for all 8)
...
- [x] Re-run the §Verification greps. (verified: no `../../../{shared,main,renderer}` and no `from 'electron'` under `src/plugins/**`; `src/main/**` imports `plugins/builtins/**` only in `main/plugins/builtinRegistry.ts` (the composition root, now uniform across all 8 plugins); `src/renderer/src/{App.tsx,components/**,plugins/registry.ts}` has zero `plugins/builtins` imports.)
