...
import {
  ...
  type ConnectionLoggerGraphMode,
  type ConnectionLoggerMainMessage,
  type ConnectionLoggerTimeWindow
} from './protocol'
import './styles.css'
...
export default function ConnectionLoggerView({ tabId, hostId }: PluginViewProps) {
  const scope = connectionLoggerScopeId(hostId, tabId)
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      if (cancelled) return
      setEvents(isConnectionLoggerData(raw) ? raw.events : [])
  ...
  useEffect(() => {
    const offMsg = window.wassh.onPluginMessage((ev) => {
      if (ev.tabId !== tabId || ev.pluginId !== PLUGIN_ID_CONNECTION_LOGGER) return
      const payload = ev.payload as ConnectionLoggerMainMessage | undefined
      if (!payload || payload.scopeId !== scope) return
