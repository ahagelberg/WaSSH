...
export default function MacroPadView({
  ...
  const send = useCallback(
    (text: string): void => {
      void window.wassh.sendPluginMessage(
        tabId,
        pluginId,
        { type: 'send', text } satisfies MacroPadRendererMessage
      )
    },
    [tabId, pluginId]
  )
  ...
  }, [buttons, tabId, pluginId, active, dialog, rowMenu, groupMenu, colorMenu, send])
